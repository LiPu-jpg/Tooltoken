...
def deterministic_requirements(query):
    """Return bounded request-clause spans without inventing semantic constraints."""
    ...
    selected = [(left, right) for left, right in sentences if DIRECT_REQUEST.search(query[left:right])]
    if not selected:
        selected = [(left, right) for left, right in sentences if FALLBACK_REQUEST.search(query[left:right])]
    if not selected:
        selected = [(0, len(query))]
    spans = []
    for left, right in selected:
        boundaries = [left]
        boundaries.extend(left + match.start() for match in ACTION_SPLIT.finditer(query[left:right]))
        boundaries.append(right)
        for start, end in zip(boundaries, boundaries[1:]):
            while start < end and (query[start].isspace() or query[start] in ",;"): start += 1
            while end > start and query[end - 1].isspace(): end -= 1
            if start < end and (start, end) not in spans:
                spans.append((start, end))
    if len(spans) > 12:
        spans = spans[:11] + [(spans[11][0], spans[-1][1])]
    return [{"source_span": [left, right], "constraints": {}} for left, right in spans]
...
class Ledger:
    ...
    def context(self):
        return {'requirements':[copy.deepcopy(vars(r)) for r in self.requirements.values()],
...
class BoundedSession:
    ...
    """
    def __init__(self, controller, generate):
    ...
    def finish(self, answer, audit):
        ...
        if route['action']=='repair_answer':
            # Token capacity was reserved by finish_route; no independent budget.
            revised=self.generate('repair_answer',{'answer':answer,'requirements':route['requirement_ids'],
                                  'evidence':self.controller.ledger.context(),'audit':route['audit']},route['token_limit'])
            if not isinstance(revised,str) or not revised.strip():
                return {'action':'honest_stop','answer':answer,'reason':'answer_repair_failed'}
            # Do not certify the rewritten answer or repeatedly request repairs.
            fresh=self.generate_bounded('audit_answer',{'answer':revised,'context':self.controller.ledger.context()},768)
            try:fresh=strict_json(fresh) if isinstance(fresh,str) else fresh
            except (ValueError,TypeError):fresh=None
            return self.controller.finish_route(revised,fresh)
        return route
    ...
    def finish_coverage(self, answer, audit, *, allow_selection_recovery=True):
        ...
        if route['action']=='coverage_answer_repair':
            revised=self.generate('repair_answer',{'answer':answer,
                                  'requirements':route['requirement_ids'],
                                  'evidence':self.controller.ledger.context(),
                                  'audit':route['audit']},route['token_limit'])
            if not isinstance(revised,str) or not revised.strip():
                return {'action':'finish','answer':answer,'reason':'coverage_answer_repair_failed'}
            return {'action':'finish','answer':revised.strip(),'reason':'coverage_answer_repaired'}
        return route
