...
def run(query, definitions, propose, generate, execute, validate, *, budget, max_decisions, max_validation_retries=2):
    ...
    if definitions is None:
        ...
        try:
            ...
            # Keep only exact source spans and no inferred constraints. This
            # fallback is structurally useful state, never semantic certification.
            ledger=Ledger(query,deterministic_requirements(query))
            extraction_fallback=True
            extraction_mode='deterministic_source_span_fallback'
    else:
        ledger=Ledger(query,definitions)
    controller=CompletionController(ledger,budget)
    session=BoundedSession(controller,generate)
    history=[{'type':'user','content':query}]
    events=[];plan=None;coverage_excluded=[];coverage_recovery_active=False;force_finish=False;last_answer=''
    def finish(status,answer='',reason=''):
        return {'status':status,'answer':answer,'reason':reason,'history':history,
                'events':events,'budget':vars(budget).copy(),'ledger':ledger.context(),
    ...
    def evidence_answer_or_stop(reason):
        if ledger.observations:
            answer=session.generate_bounded('evidence_answer',{'original_query':query,'context':ledger.context()},1024)
            if isinstance(answer,str) and answer.strip():
                events.append({'action':'partial_answer','reason':reason,'observation_count':len(ledger.observations)})
                return finish('give_answer',answer.strip(),reason+'_evidence_answer')
        return finish('give_up_and_restart',reason=reason)
