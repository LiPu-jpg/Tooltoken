"""Opt-in source-patch controller; requires explicit reader mode and fresh output."""
...
def normalize_trace(result, receipts, seconds, executions):
    ...
    return {
        ...
        "history": history,
        "costs": {
            # Keep the old field comparable but disclose its wall-time semantics.
            "model_seconds": seconds,
            "model_seconds_definition": "legacy_episode_wall_including_executor",
            "episode_wall_seconds": seconds,
            "generation_seconds": sum(row.get("seconds",0.0) for row in result.get("native_diagnostics", [])
                                      if row.get("kind") in {"native_generation", "auxiliary_generation"}),
            ...
            "executions_without_receipt": result["budget"]["calls"] - len(receipts),
            "validation_failures": sum(event.get("action") in {"rejected_schema", "rejected_generation_arguments"} for event in result["events"]),
            "validation_retries": sum(event.get("action") in {"rejected_schema", "rejected_generation_arguments"} for event in result["events"]),
            "validation_retries_definition": "legacy_rejected_proposal_count_including_terminal_rejection",
            "generation_capacity_tokens": result["budget"]["tokens"],
        },
        "execution_receipts": receipts,
        "decisions": result["events"],
        "native_diagnostics": result.get("native_diagnostics", []),
        "completion_controller": {
            "version": "source-patch-native-fallback-v6",
...
def main() -> None:
    ...
    parser.add_argument("--reader-mode",choices=["memory_fp32","documents_fp32"],required=True)
    ...
    if (args.max_thought_tokens,args.max_argument_tokens,args.max_answer_tokens)!=(32,256,1024):
        raise ValueError("This adapter implements 32/256/1024; do not silently ignore CLI budgets")
    ...
    install_reader(agent, tools, args.reader_mode)
    if agent.reader_variant != args.reader_mode:
        raise ValueError("Requested reader was not installed")
    (args.output_dir / "RUN_CONFIGURATION.json").write_text(json.dumps({
        "controller":"source-patch-native-fallback-v6", "reader_mode":args.reader_mode,
        "actual_limits":vars(agent.limits), "slots":agent.slots,
        ...
        "registry_count_including_finish":len(tools),
        "executor_config_sha256":sha256(args.executor_config),
        "runner_sha256":sha256(HERE),
        "variant_sources_sha256":{str(p.relative_to(ADAPTER)):sha256(p)
                                 for p in sorted((ADAPTER / "variant").rglob("*.py"))},
        "optimizer_updates":0, "task_success_judged":False,
    },indent=2)+"\n")
    ...
    with ExitStack() as stack:
        ...
        for query in queries:
            ...
            policy["answer_patch_applied"] += int(any(event.get("outcome") == "source_patch_applied" for event in trace["decisions"]))
            policy["answer_patch_preserved"] += int(any(event.get("action") == "evidence_patch" and event.get("outcome") != "source_patch_applied" for event in trace["decisions"]))
    ...
    report = {
        ...
        "controller": "source-patch-native-fallback-v6",
        "programmatic_finish_adapter": True,
        "task_success_judged": False,
        "official_sopr": False,
        "optimizer_updates": 0,
    ...
    provenance = {
        ...
        "completion_source_sha256": {
            ...
            for name in ("completion.py", "runner.py", "native_adapter.py", "prompts.py", "evidence_patch.py")
        },
