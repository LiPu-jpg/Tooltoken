...
class NativeSequenceSFT(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # Padding can leave a rank with no positive examples in the final
        # global batch.  All ranks must still execute the complete backbone
        # path above so ZeRO-3 observes the same parameter all-gather order;
        # the zero weights then remove padded rows from the loss.
